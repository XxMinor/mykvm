...
- **Linux。** 键鼠共享目前需要 **Xorg/X11 会话**；程序会识别原生 Wayland，但暂不支持。请在登录界面选择 Xorg 会话并重启 MyKVM。使用 AppImage 时还需先添加可执行权限（`chmod +x`）。

## 已知限制

- **仅限可信局域网。** 设备配对需要确认码，但局域网发现仍是明文、未认证。请勿把端口暴露到公网或不可信网络。
- 输入和剪贴板走 **加密的 QUIC/TLS** 连接，并由已保存的配对密钥和证书绑定授权；但 MyKVM 未针对恶意网络做加固。
- 剪贴板同步 **文本和图片**，不同步文件。Linux 遇到同一富剪贴板项目同时含图片和文本时，目前优先保留图片，并在日志中明确记录文本格式降级。
...
| 输入 | UDP `47834` | QUIC datagram + 有序 stream | `mykvm.input.v1` | 鼠标移动保留最新帧，按键、鼠标键、滚轮和切换边界可靠有序传输 |
