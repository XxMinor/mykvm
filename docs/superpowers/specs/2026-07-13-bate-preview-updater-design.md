...
Publish the current feature work through the `bate` preview branch and make preview builds update only from `aceleisureman/mykvm`'s beta release channel.

## Design

- Keep `main` as the stable release channel.
- Treat pushes to `bate` as beta releases with versions such as `0.9.9-beta.<run>`.
...
- Move the current branch's completed changes onto `bate` without including the untracked `claude_auto_continue.py` file.
